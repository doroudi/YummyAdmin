...
import type { ForgetPasswordResponse, ForgetPasswordViewModel, LoginResponse, LoginViewModel, RegisterResponse, RegisterViewModel } from '~/models/Account'
...
const handlers = [
  ...
  http.post('*/api/account/register', async ({ request }) => {
    ...
    if (user.username.toLowerCase() === 'yummy')
      return HttpResponse.json(null, { status: 400, statusText: 'user name is already taken!' })

    const response: RegisterResponse = { token: 'JWT_Fake_Token', isSucceed: true }
    await delay(1000)
    return HttpResponse.json(response, { status: 200 })
  }),
  ...
  http.post('*/api/account/forget-password', async ({ request }) => {
    ...
    if (user.email !== 'yummy@admin.io')
      return HttpResponse.json(null, { status: 400, statusText: 'Email is not found' })
